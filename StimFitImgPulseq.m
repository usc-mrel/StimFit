function [T2, B1, amp, opt] = StimFitImgPulseq(img,opt)
...
switch opt.lsq.Ncomp
    case 1
        ...
        amp   = zeros(opt.size(1:3));
    case 2
        T2    = zeros([opt.size(1:3) 2]);
        B1    = zeros([opt.size(1:3) 2]);
        amp   = zeros([opt.size(1:3) 2]);
    case 3
        T2    = zeros([opt.size(1:3) 3]);
        B1    = zeros([opt.size(1:3) 3]);
        amp   = zeros([opt.size(1:3) 3]);
end
...
for nsi = 1:opt.size(3)
    ...
    if all(img(npi,nvi,nsi,opt.th_te) > th)
        ...
        S = img(npi,nvi,nsi,:);
        [T2(npi,nvi,nsi,:) B1(npi,nvi,nsi,:) amp(npi,nvi,nsi,:) opt] = StimFit(S(:),opt);
